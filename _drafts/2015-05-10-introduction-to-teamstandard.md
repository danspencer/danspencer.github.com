...
I consider myself fortunate to have come into employment situations where a development team has already developed standardized libraries, be it for front-end or back-end, which made application development a lot easier and faster. Having a set of standardized libraries for application development helps bring cohesion, in my opinion, amongst a development team's applications. The reason for this (and future) blog posts is to share what I've found and developed as some answers weren't easily found online.

Recently, my development team switched to [.Net MVC 5](http://www.asp.net/mvc) from [Ruby on Rails](http://rubyonrails.org). Quickly, it was apparent to me that someone needed to standardize elements of our applications in order to unify common features/functionality of our applications. We had created gems in-house which had done so and had worked well which provided me with a roadmap of things we should standardize in our [.Net MVC](http://www.asp.net/mvc) applications.

Since [NuGet](https://www.nuget.org) packages appeared to be the [.Net MVC](http://www.asp.net/mvc) equivalent to gems in [Ruby on Rails](http://rubyonrails.org), I decided that would be the best way to deliver any standardization to our projects.

I decided for my first foray into [Nuget](https://www.nuget.org) package creation, I would simply implement our custom [Bootstrap](http://getbootstrap.com) theme in a [.Net MVC](http://www.asp.net/mvc). Thus, [TeamStandard.Bootstrap](https://github.com/danspencer/TeamStandard.Bootstrap) was created.

The goal of TeamStandard.Bootstrap is simply to copy over the custom Bootstrap theme CSS file and update `BundlerConfig.cs` so that the custom Bootstrap CSS theme file is loaded.


Our development team has recently switched over to .Net MVC and it was evident from the start that everyone had different ways of doing thing be it. Prior to our switch to development, we were using Ruby on Rails and had developed over time standardize gems to implement various features for our applications such as authentication, authorization, monitoring, etc ... Having gems for all these features made implementing these features quick and easy. Another benefit to having these standardized libraries, was that it made it easier to maintain the code. I decided to take the initiative to do something similar for .Net MVC which introduced me to NuGet packages.
